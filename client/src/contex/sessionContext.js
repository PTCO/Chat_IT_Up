...
export const SessionProvider = (props) => {
    ...
    const createSession = async(Username, UserTwoID, UserTwoPortrait, Request) => {
        await axios.post('http://localhost:4000/Sessions/Create', 
        {
    ...
    const deleteSession = async(Session_ID)=> {
        await axios.delete(`http://localhost:4000/Sessions/Delete/${Session_ID}/${authUser.User_ID}`)
        .then(result => setSessions(result.data))
        .catch( errors => {
            handleErrors(errors);
        })
    ...
    const searchSession = async(e, search) => {
        ...
        await axios.get(`http://localhost:4000/Sessions/Search/${search}/CurrentUser/${authUser.Username}/${authUser.User_ID}`)
    ...
    const getMessages = async () => {
        if(session && session.Session_ID !== undefined){
            await axios.get('http://localhost:4000/Sessions/Messages/' + session.Session_ID)
